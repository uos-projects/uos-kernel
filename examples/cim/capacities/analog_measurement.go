...
func (c *AnalogMeasurementCapacity) convertMQMessage(mqMsg *mq.MQMessage) capacities.Message {
...
// CanHandle 检查是否能处理消息
func (c *AnalogMeasurementCapacity) CanHandle(msg capacities.Message) bool {
	_, ok := msg.(*AnalogMeasurementValueMessage)
	return ok
}

// Execute 处理测量值消息（从 MQ 接收）
func (c *AnalogMeasurementCapacity) Execute(ctx context.Context, msg capacities.Message) error {
