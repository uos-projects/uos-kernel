...
type ActorRefSetter interface {
	SetActorRef(actorRef interface {
		Send(msg capacities.Message) bool
	})
}

// BaseMeasurementCapacity 提供 Measurement Capacity 的基础实现（CIM 特定）
...
type BaseMeasurementCapacity struct {
	*capacities.BaseCapacity
	...
	capacityRef capacities.Capacity
	ctx         context.Context // 用于 Execute 调用
}

// NewBaseMeasurementCapacity 创建基础 Measurement capacities.Capacity
func NewBaseMeasurementCapacity(
	...
	return &BaseMeasurementCapacity{
		BaseCapacity:    capacities.NewBaseCapacity(name, measurementID),
...
func (b *BaseMeasurementCapacity) SetCapacityRef(capacity capacities.Capacity, ctx context.Context) {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *BaseMeasurementCapacity) StartConsumeLoop(
	ctx context.Context,
	topic string,
	messageConverter func(*mq.MQMessage) capacities.Message,
) {
	go func() {
		for {
			select {
