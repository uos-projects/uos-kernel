...
type CIMResourceActor struct {
	*actors.BaseResourceActor // 嵌入基础资源 Actor（能力管理）
	...
	runtimeContext *actors.RuntimeContext
...
func NewCIMResourceActor(
	...
	runtimeContext := actors.NewRuntimeContext(stateBackend)
...
func (a *CIMResourceActor) GetRuntimeContext() *actors.RuntimeContext {
...
func (a *CIMResourceActor) CreateSnapshot(sequence int64) (*CIMSnapshot, error) {
...
// Receive 重写消息接收逻辑，处理 CIMResourceActor 特有的消息
func (a *CIMResourceActor) Receive(ctx context.Context, msg actors.Message) error {
	// 处理 CIMResourceActor 特有的消息
	switch m := msg.(type) {
	case *actors.SetPropertyMessage:
		// 通过消息设置属性
		a.setProperty(m.Name, m.Value)
		return nil
	case *actors.GetPropertyMessage:
		// 获取属性（这里简化处理，实际可以通过响应消息返回）
		_, _ = a.getProperty(m.Name)
		return nil
	}

	// 其他消息交给基类处理（Capacity 路由、状态管理等）
	return a.BaseResourceActor.Receive(ctx, msg)
}

// RestoreFromSnapshot 从快照恢复状态
func (a *CIMResourceActor) RestoreFromSnapshot(snapshot *CIMSnapshot) error {
	// 恢复属性（内部方法，快照恢复是特殊情况）
...
// CIMSnapshot CIM 特定的快照（包含 OWL URI）
type CIMSnapshot struct {
	ActorID     string
	OWLClassURI string
	Sequence    int64
	Timestamp   time.Time
