...
func NewBreakerSwitchingCapacity(breaker *BreakerActor) *BreakerSwitchingCapacity {
	return &BreakerSwitchingCapacity{
		BaseCapacity: capacities.NewBaseCapacity("BreakerSwitchingCapacity", breaker.ResourceID()),
		breaker:      breaker,
	}
}
...
func (c *BreakerSwitchingCapacity) CanHandle(msg capacities.Message) bool {
	switch msg.(type) {
...
func (c *BreakerSwitchingCapacity) Execute(ctx context.Context, msg capacities.Message) error {
	binding, ok := c.breaker.GetBinding(actors.BindingTypeDevice)
	if !ok {
		return nil
	}
