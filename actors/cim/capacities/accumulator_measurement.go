...
func (c *AccumulatorMeasurementCapacity) convertMQMessage(mqMsg *mq.MQMessage) capacities.Message {
...
// CanHandle 检查是否能处理消息
func (c *AccumulatorMeasurementCapacity) CanHandle(msg capacities.Message) bool {
	_, ok := msg.(*AccumulatorMeasurementValueMessage)
	return ok
}

// Execute 处理测量值消息（从 MQ 接收）
func (c *AccumulatorMeasurementCapacity) Execute(ctx context.Context, msg capacities.Message) error {
