...
func (c *DiscreteMeasurementCapacity) convertMQMessage(mqMsg *mq.MQMessage) Message {
...
// CanHandle 检查是否能处理消息
func (c *DiscreteMeasurementCapacity) CanHandle(msg Message) bool {
	_, ok := msg.(*DiscreteMeasurementValueMessage)
	return ok
}

// Execute 处理测量值消息（从 MQ 接收）
func (c *DiscreteMeasurementCapacity) Execute(ctx context.Context, msg Message) error {
